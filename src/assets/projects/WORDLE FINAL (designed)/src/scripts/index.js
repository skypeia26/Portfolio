...
const startTimer = () => {
  ...
  const updateTimer = () => {
    ...
    timerElement.textContent = `Timer: ${secondsElapsed} seconds`
  };
...
const resetGame = () => {
  ...
  mainBody.style.background = "linear-gradient(90deg, rgba(20,195,37,1) 0%, rgba(0,0,0,1) 30%, rgba(0,0,0,1) 70%, rgba(20,195,37,1) 100%)";
  guessAttempts = 0;
  ...
  history.length = 0;
  currentWord = '';
  WORD_OF_THE_DAY = generateNewSecretWord();  
...
let WORD_OF_THE_DAY = generateNewSecretWord(); 
...
const checkGuess = (guess, word) => {
  ...
  if (wordLetters.join('') === guess) {
    alert('You guessed the word in '+guessAttempts+' attempts!');
...
function changeBackground(numberGuesses){
  let mainBody = document.getElementById("main-body");
  if(numberGuesses===1){
    mainBody.style.background = "linear-gradient(90deg, rgba(153,195,20,1) 0%, rgba(0,0,0,1) 30%, rgba(0,0,0,1) 70%, rgba(153,195,20,1) 100%)";
    return;
  }
  else if(numberGuesses===2){
    mainBody.style.background = "linear-gradient(90deg, rgba(195,172,20,1) 0%, rgba(0,0,0,1) 30%, rgba(0,0,0,1) 70%, rgba(195,172,20,1) 100%)";
    return;
  }
  else if(numberGuesses===3){
    mainBody.style.background = "linear-gradient(90deg, rgba(195,122,20,1) 0%, rgba(0,0,0,1) 30%, rgba(0,0,0,1) 70%, rgba(195,122,20,1) 100%)";
    return;
  }
  else if(numberGuesses===4){
    mainBody.style.background = "linear-gradient(90deg, rgba(195,72,20,1) 0%, rgba(0,0,0,1) 30%, rgba(0,0,0,1) 70%, rgba(195,72,20,1) 100%)";
    return;
  }
  else if(numberGuesses===5){
    mainBody.style.background = "linear-gradient(90deg, rgba(195,20,50,1) 0%, rgba(6,0,3,1) 30%, rgba(0,0,0,1) 70%, rgba(195,20,50,1) 100%)";
    return;
  }
}
...
const onKeyDown = (key) => {
  ...
  if (key === ENTER_KEY) {
    if (currentWord.length < 5) {
      showMessage("Five letter words only.");
      return;
    }

    if (currentWord.length === 5 && WORD_LIST.includes(currentWord)) {
      if (history.includes(currentWord)) {
        showMessage('Try a different word.');
      } else {
        guessAttempts += 1 ;
        console.log("number of attempts "+guessAttempts);
        changeBackground(guessAttempts);
        checkGuess(currentWord, WORD_OF_THE_DAY);
      }
    } else {
      currentRow.setAttribute('data-animation', 'invalid');
      showMessage("Enter a real English word.");
    }
